import type {
  ...
  TypeVariant
} from "../../template-registry/dist/index.js";

import type { RuntimeDependencyRecoveryContext } from "./runtime-dependencies.js";
import { detectPackageManager, globalInstallArgsFor } from "./package-manager.js";

export type { DevServer, GenerateKind, ScaffoldMode, ScaffoldProjectMetadata, TypeVariant };
...
type TemplateRegistryModule = typeof import("../../template-registry/dist/index.js");
...
const isMissingTemplateRegistryError = (error: unknown): boolean => {
...
export const loadScaffoldTemplateRegistry = async (
  context?: RuntimeDependencyRecoveryContext,
  importer: () => Promise<TemplateRegistryModule> = async () => {
    try {
      return (await import(TEMPLATE_REGISTRY_PACKAGE)) as TemplateRegistryModule;
    } catch {
      return (await import(WORKSPACE_TEMPLATE_REGISTRY_PATH)) as TemplateRegistryModule;
    }
  }
): Promise<TemplateRegistryModule | null> => {
  try {
    ...
    if (!context) {
      throw new Error(
        `${missingDependencyMessage} Install it with npm install -g @sculptor/template-registry.`
      );
    }

    const answer = await context.prompt(
      `${missingDependencyMessage}\n\nWould you like to install it and rerun the command? (y/n)`,
      "n"
    );
    ...
    const installResult = context.spawn(
      packageManager,
      globalInstallArgsFor(packageManager, [TEMPLATE_REGISTRY_PACKAGE]),
      {
      cwd: context.cwd,
      stdio: "inherit",
    ...
    try {
      ...
      throw new Error(
        `${missingDependencyMessage} The package was installed, but it could not be loaded afterward.`
      );
    }
  }
};
...
export const generateResourceFiles = async (
  kind: GenerateKind,
  name: string,
  mode: ScaffoldMode,
  ...
  return registry.generateResourceFiles(
    kind,
    name,
    mode,
    devServer,
...
export const syncTestHarness = async (
  targetDir: string,
  context?: RuntimeDependencyRecoveryContext
): Promise<boolean> => {
  const registry = await loadScaffoldTemplateRegistry(context);
