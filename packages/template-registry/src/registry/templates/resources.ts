...
export const createRepositoryResource = (name: string): Record<string, string> => ({
...
export const createDtoResource = (name: string): Record<string, string> => ({
...
export const createPackageResource = (
  ...
  const routePrefix =
    options?.routePrefix ?? (mode === "hybrid" ? `r/${routeName}` : `${toRoutePath(routeName)}/route`);
