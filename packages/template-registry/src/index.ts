...
const resolveFileStem = (name: string | undefined, outputDir: string): string => {
  if (name) {
    return name;
  }
...
const controllerFileName = (name: string): string => `${name}.controller.ts`;
const serviceFileName = (name: string): string => `${name}.service.ts`;
const moduleFileName = (name: string): string => `${name}.module.ts`;
const middlewareFileName = (name: string): string => `${name}.middleware.ts`;
const routeFileName = (name: string): string => `${name}.routes.ts`;
const typeFileName = (name: string, variant: TypeVariant): string => {
  const suffix = variant === "type" ? "type" : variant;
  return `${name}.${suffix}.ts`;
};

const specFileName = (name: string, suffix: string): string => `${name}.${suffix}.spec.ts`;

const specImportPath = (sourcePath: string): string =>
  normalizeRelativePath(path.posix.relative("src/tests", sourcePath)).replace(/\.ts$/, ".js");

const resolveGeneratorOutputDir = (
  kind: GenerateKind,
  outputDir?: string
): string => {
  if (outputDir) {
...
const devScriptFor = (devServer: DevServer): string =>
  devServer === "tsx"
    ? "tsx src/main.ts"
    : "nodemon --watch src --ext ts --exec tsx src/main.ts";

const devDependenciesFor = (devServer: DevServer): string =>
  devServer === "tsx"
    ? ""
    : `    "nodemon": "^3.1.9",\n`;
...
const registryTemplate = (mode: ScaffoldMode): string => {
  if (mode === "decorator") {
...
const serviceSpecTemplate = (name: string, sourcePath: string): string => {
  const importPath = specImportPath(sourcePath);
  return `import { describe, expect, it } from "vitest";
...
const routeSpecTemplate = (name: string, sourcePath: string): string => {
  const importPath = specImportPath(sourcePath);
...
const middlewareSpecTemplate = (name: string, sourcePath: string): string => {
  const importPath = specImportPath(sourcePath);
...
const createFunctionalArtifacts = (name: string): Record<string, string> => ({
...
const createServiceResource = (name: string): Record<string, string> => ({
  [`src/app/services/${serviceFileName(name)}`]: `export class ${toPascalCase(name)}Service {}
`
});

const createModuleResource = (name: string): Record<string, string> => ({
  [`src/app/modules/${moduleFileName(name)}`]: `export class ${toPascalCase(name)}Module {}
`
});

const createMiddlewareResource = (name: string): Record<string, string> => ({
...
const createRouteResource = (name: string): Record<string, string> => ({
  [`src/app/routes/${routeFileName(name)}`]: `import { Router } from "express";
...
const createHandlerResource = (name: string): Record<string, string> => ({
  [`src/app/handlers/${name}.handler.ts`]: `export const ${toPascalCase(name)}Handler = (
  _req: unknown,
  res: { json: (value: unknown) => void }
) => {
  ...
  switch (kind) {
    case "controller":
      ...
      if (functionalRoutes) {
        Object.assign(sourceFiles, {
          [`${functionalRouteDir}/${routeFileName(resolvedName)}`]:
            createFunctionalArtifacts(resolvedName)[
              `src/app/routes/${routeFileName(resolvedName)}`
            ],
          [`${functionalHandlerDir}/${resolvedName}.handler.ts`]:
            createFunctionalArtifacts(resolvedName)[`src/app/handlers/${resolvedName}.handler.ts`]
        });
      }
      break;
    case "service":
      Object.assign(sourceFiles, {
        [`${resolvedOutputDir}/${serviceFileName(resolvedName)}`]:
          createServiceResource(resolvedName)[`src/app/services/${serviceFileName(resolvedName)}`]
      });
      break;
    case "module":
      Object.assign(sourceFiles, {
        [`${resolvedOutputDir}/${moduleFileName(resolvedName)}`]:
          createModuleResource(resolvedName)[`src/app/modules/${moduleFileName(resolvedName)}`]
      });
      break;
    case "middleware":
      Object.assign(sourceFiles, {
        [`${resolvedOutputDir}/${middlewareFileName(resolvedName)}`]:
          createMiddlewareResource(resolvedName)[
            `src/app/middlewares/${middlewareFileName(resolvedName)}`
          ]
      });
      break;
    case "type":
    ...
    case "route":
      if (mode === "functional" || mode === "hybrid") {
        Object.assign(sourceFiles, {
          [`${resolvedOutputDir}/${routeFileName(resolvedName)}`]:
            createRouteResource(resolvedName)[`src/app/routes/${routeFileName(resolvedName)}`]
        });
      }
      break;
  }
