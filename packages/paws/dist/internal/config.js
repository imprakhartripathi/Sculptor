...
export const resolveLoggingConfig = (rootDir = process.cwd()) => ({
    enabled: asBoolean(getConfig("logging.enabled", rootDir), true),
    dogMode: asBoolean(getConfig("logging.dogMode", rootDir), false)
});
export const resolveLoggerMode = (rootDir = process.cwd()) => resolveLoggingConfig(rootDir).dogMode ? "dog" : "standard";
